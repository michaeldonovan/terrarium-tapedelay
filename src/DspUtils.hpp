 ...
 */
inline constexpr float fastexp3(float x) { return (6 + x * (6 + x * (3 + x))) * 0.16666666f; }
...
{
    return (5040 + x * (5040 + x * (2520 + x * (840 + x * (210 + x * (42 + x * (7 + x))))))) *
           0.00019841269f;
}
...
inline constexpr float fastexp8(float x)
{
    return (40320 +
            x * (40320 +
                 x * (20160 + x * (6720 + x * (1680 + x * (336 + x * (56 + x * (8 + x)))))))) *
           2.4801587301e-5;
}
...
inline constexpr float fastexp9(float x)
{
    return (362880 +
            x * (362880 +
                 x * (181440 +
                      x * (60480 +
                           x * (15120 + x * (3024 + x * (504 + x * (72 + x * (9 + x))))))))) *
           2.75573192e-6;
}
...
/**
 * @brief Remaps an input value in the range [old_min, old_max] to range
 * [new_min, new_max]
 */
template <class Numeric_t>
inline constexpr Numeric_t remap(Numeric_t in, Numeric_t old_min, Numeric_t old_max,
                                 Numeric_t new_min, Numeric_t new_max)
{
 ...
 */
template <class Numeric_t>
inline constexpr Numeric_t remap_exp(Numeric_t in, Numeric_t old_min, Numeric_t old_max,
                                     Numeric_t new_min, Numeric_t new_max)
{
...
{
public:
    dc_blocker(Float_t sample_rate, Float_t freq_hz = 20) : fs_(sample_rate)
    {
